<flux:card class="p-0!">
    ...
    <flux:modal name="edit-section-modal-{{ $section['id'] }}">
        <div class="space-y-6">
            ...
            <flux:input label="{{ __('Section Name') }}" placeholder="{{ __('Main Content') }}" wire:model="form.tabs.{{ $tabIndex }}.sections.{{ $sectionIndex }}.name" />
            <flux:textarea label="{{ __('Section Instructions') }}" placeholder="{{ __('Instructions for this section...') }}" rows="3" wire:model="form.tabs.{{ $tabIndex }}.sections.{{ $sectionIndex }}.instructions" />
            <div class="flex justify-end">
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                @foreach ($this->fieldTypeMeta as $type)
                    <flux:card :key="$type['value']" wire:click="addField({{ json_encode(['tabIndex' => $tabIndex, 'sectionIndex' => $sectionIndex, 'type' => $type['value']]) }})" class="cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800">
